...
if(forms){
    forms.addEventListener('input', async (e)=>{
        ...
        try {
            ...
            class Insert{
                constructor(nameEl, cardNum, month, year, cvc){
                    this.nameEl = nameEl
                ...
                putting(){
                    nameEl.textContent = this.nameEl
            ...
            class Form extends Insert{
                async createCard(){
                    const card = new Form(nameEl, cardnum, month, year, cvc)
                    console.log(card)
                }
            }
            const form = new Form(nameEl, cardNumber, month, year, cvc)
            await form.createCard();
            const insert = new Insert(nameEl, cardNumber, month, year, cvc)
            await insert.createCard();
            return form
        } catch (error) {
