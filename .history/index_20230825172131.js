...
if(forms){
    forms.addEventListener('input', async (e)=>{
        ...
        try {
            ...
            class Insert{
                constructor(name, cardNum, month, year, cvc){
                    this.name = name
                    this.cardnum= cardNum;  parseInt(cardNum);
                    this.month = month;  parseInt(month);
                    this.year = year;  parseInt(year);
                    this.cvc = cvc;  parseInt(cvc);
                }
                putting(){
                    nameEl.textContent = this.name
                    number.textContent = this.cardnum
                    expDate.textContent = this.month + `/` + this.year
                    cvcDigit.textContent = parseInt(cvcVal.value)
                }
            }
            class Form extends T{
                constructor(name, cardNum, expDate, cvc){
                    this.name = name
                    this.cardnum= cardNum;  parseInt(cardNum);
                    this.expDate = expDate; parseInt(expDate)
                    this.cvc = cvc;  parseInt(cvc);
                }
                async createCard(){
                    const card = new Form(this.name, this.cardnum, this.expDate, this.cvc)
                    console.log(card)
                }
            }
            const form = new Form(nameVal.value, numberVal.value, monthVal.value, cvcVal.value)
