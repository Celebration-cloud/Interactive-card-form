...
const puttingInfo = () =>{
    ...
    Math.max(monthVal.value = 12)
    Math.min(monthVal.value = -1)
    const yea = parseInt(yearVal.value)
    Math.max(yearVal.value = 35)
    /* `Math.min(yearVal.value = 24)` is setting the value of `yearVal` to 24 and then returning the
    minimum value between the current value of `yearVal` and 24. It is essentially ensuring that the
    value of `yearVal` is not less than 24. */
    Math.min(yearVal.value = 24)
